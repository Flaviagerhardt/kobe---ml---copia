...
st.markdown("### Previsão de Arremessos de Kobe Bryant (PyCaret + Kedro + MLflow)")
...
def carregar_modelos():
    ...
    for nome, caminho in arquivos_modelo.items():
        try:
            if caminho.exists():
                # Usar pickle para carregar o modelo PyCaret
...
if page == "Visão Geral":
    ...
    O pipeline foi construído usando Kedro para orquestração, PyCaret para treinamento de modelos, e MLflow para 
...
# Página de Desempenho do Modelo
    ...
    st.markdown("""
    ### Comparação de Modelos (PyCaret)
    Nesta seção, comparamos o desempenho dos dois modelos treinados usando PyCaret:
    - Regressão Logística
    - Árvore de Decisão
    """)
    ...
    if metricas_mlflow:
        ...
        if runs_treinamento:
            ...
            if test is not None and "modelo_final" in modelos:
                ...
                # Usar PyCaret para fazer previsões em algumas amostras
                try:
                    ...
                    # Fazer previsões
                    preds = predict_model(modelos["modelo_final"], data=test_amostra)
                    
                    # Ajustar nomes das colunas para visualização
                    preds_view = test_amostra.copy()
                    preds_view["Probabilidade"] = preds["Score_1"]
                    preds_view["Previsão"] = preds["Label"]
                    preds_view["Real"] = test_amostra["shot_made_flag"]
                    ...
                    st.error(f"Erro ao fazer previsões com PyCaret: {e}")
        else:
...
# Página de Monitoramento de Produção
    ...
    if predicoes is not None:
        ...
        if "shot_made_flag" in predicoes.columns and pred_col:
            ...
            st.dataframe(erros_confiantes[["shot_distance", "period", "playoffs", "shot_made_flag", pred_col, prob_col]])
        else:
...
# Página de Análise de Drift
    ...
    if train is not None and prod is not None:
        ...
        # Mostrar resumo
        drift_percentage = (features_com_drift / len(features)) * 100
        significant_drift = drift_percentage > 30  # Se mais de 30% das features têm drift
        
        col1, col2 = st.columns(2)
        col1.metric("Features com Drift", f"{features_com_drift}/{len(features)}")
...
st.markdown("Desenvolvido com PyCaret, Kedro e MLflow - Projeto de Disciplina de Engenharia de Machine Learning")